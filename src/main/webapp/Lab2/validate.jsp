...
<jsp:useBean id="mine" class="com.LiuJunwei.Lab2.Login" scope="request" ></jsp:useBean>

    <%--Todo 2: Use <jsp:setProperty> to set  beans' property username and password--%>
<jsp:setProperty name="mine" property="username" value='<%=request.getParameter("username")%>'/>
<jsp:setProperty name="mine" property="password" value='<%=request.getParameter("password")%>'/>

<%
    if(mine.getUsername().equals("admin") && mine.getPassword().equals("admin")){
        request.getSession().setAttribute("username",mine.getUsername());
